...
public class LITPvolmgrapiTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPvolmgrapiTestRunner.class);
    ...
    @TestId(id = "CXP9031137-1", title = "Upgrade ERIClitpvolmgrapi rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitpvolmgrapiRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("volmgrapi");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031137-2", title = "Run python test cases for ERIClitpvolmgrapi package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitpvolmgrapiTests() {
     ...
     * @PRE Execution of test {@link #runERIClitpvolmgrapiTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031137-3", title = "Parse xml outputs from python tests")
